...
public class InputDataGenerator extends Dialog{
	SVM svm;
	TextField attributes_count, vectors_count, min, max, classes_count, margin, radius; 
	Checkbox liniar;
	Label attributes_count_label, vectors_count_label, min_label, max_label, classes_count_label, liniar_label, margin_label, radius_label;
	Button generate, save;
	public TextArea ta;
	String dir = ".\\svm\\data", path;
	
	public InputDataGenerator(SVM svm){
		super(svm, "Input Data", true);
		this.svm = svm;
		...
		margin_label.setBounds(x1,y,150,20);
		margin_label.setForeground(Color.white);
		add(margin_label);
		margin = new TextField("10");
		margin.setBounds(x1+150,y,100,20);
		add(margin);

		radius_label = new Label("Radius:");
		radius_label.setBounds(x2,y,150,20);
		radius_label.setForeground(Color.white);
		add(radius_label);
		radius = new TextField("200");
		radius.setBounds(x2+150,y,100,20);
		add(radius);	
		radius.enable(false);		
	...
	void generateData(){
		...
		if(liniar.getState()){
			...
			generateNonLiniarData_1(marg, 30);  // de introdus procentul in interfata !!
			...
			float R = 100f; 
			try{
				float r = Float.parseFloat(radius.getText());
				if (r > 0) R = r;
			}			
			catch(NumberFormatException e){}
			generateCircularData(R, marg);	
			*/
	...
	void generateCircularData(float r, float marg){ //the margin: marg>=0
		...
		ta.append("@data\n");
		if(N > 1 && n > 1 && MIN < MAX && marg > 0 && r > marg){
			float[] x0 = new float[n];
			for(int i=0; i<n; i++)
				x0[i] = MIN + (float)Math.random()*(MAX-MIN);
			for(int k=0; k<N; k++){
				while(true){
					String s = "";
					float[] x = new float[n];
	...
	void generateNonLiniarData_1(float marg, int procent){ //the margin: marg>=0, procent = procent din N - indica nr de vectori aflati intre cele doua hiperplane suport
		int N = Integer.parseInt(vectors_count.getText());
		int P = procent * N / 100;
		int p = 0;
		int n = Integer.parseInt(attributes_count.getText());
