...
public class OneVersusOne extends Algorithm{
	SVM svm;
	ArrayList<Vector> v;//multimea de probe
	Clasa CP,CM;//f(x)
	...
	static class S_and_FKL{
		public int k;
		public int l;
		public KernelGaussian kg;
		
		public S_and_FKL(int k,int l, KernelGaussian kg){
			this.k=k;
			this.l=l;
			this.kg=kg;
		}
		...
		while(it < maxit){//number of iterations less than maximum
			...
			for(i = 0; i < N; i++){
				...
				for(k = 0; k < N; k++){	
					...
					Ei += alpha[k] * y_changed_k *KGaussian(v_i.X, v_k.X);
				}
	...
	int classify(float[] in){
		...
		for(int i=0;i<S.length;i++){
			KernelGaussian kgs=S[i].kg;
			float val=evaluate(in,kgs);
			if(val>=0){
				s[S[i].k]+=val;
			}else{
				s[S[i].l]+=val;
	...
	public void run(){
		...
		int p=0;
		S=new S_and_FKL[(nC*(nC+1))/2];
		for(int k=0;k<nC-1;k++){
			for(int l=k+1;l<nC;l++){
				v.clear();
				for(int i=0;i<svm.ind.V.length;i++){
					if(svm.ind.V[i].cl.Y==k){
						v.add(new Vector(svm.ind.V[i].X,CP));//adaugam vectorii 
					}else if(svm.ind.V[i].cl.Y==l){
						v.add(new Vector(svm.ind.V[i].X,CM));//adaugam vectorii 
					}
			}
			param(v);//calculam datele SMO
			KernelGaussian kg=new KernelGaussian(v,alpha,vS,bias,N);
			S[p]=new S_and_FKL(k,l,kg);
			p++;
			}
